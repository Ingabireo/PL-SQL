create or replace package branch_pkg_sec as
	procedure branch_details_pro(br_name branch.name%type, month item_sold.month_sold%type);
	procedure break_down_pro(br_name branch.name%type, month item_sold.month_sold%type);
	procedure price_detail_pro(br_name branch.name%type, month item_sold.month_sold%type);
	procedure total_sales_pro(br_name branch.name%type, month item_sold.month_sold%type);
	procedure other_branches_pro(br_name branch.name%type, month item_sold.month_sold%type);
	procedure print_report(br_name branch.name%type, month item_sold.month_sold%type);
	function check_branch_Exists_fun(br_name branch.name%type) return boolean;
	function check_sales_fun(br_name branch.name%type, month item_sold.month_sold%type) return boolean;
end branch_pkg_sec;
/


create or replace package body branch_pkg_sec as

	function check_sales_fun(br_name branch.name%type, month item_sold.month_sold%type) return boolean is
		no_items number;
		begin
			select count (Customers.id)  into no_items
				...
				where branch.name = br_name and month_sold = month;
			if(no_items > 0 ) then 
				return true;
				else
					return false;
	...
	function check_branch_Exists_fun(br_name branch.name%type) return boolean is 

		check_var int;
		begin
			select count(id) into check_var from branch where branch.name = br_name;
	...
	procedure branch_details_pro(br_name branch.name%type, month item_sold.month_sold%type) is 
	...
	procedure break_down_pro(br_name branch.name%type, month item_sold.month_sold%type) is 
		...
		begin
			...
			check_sold_var := check_sales_fun(br_name,month);
	...
	procedure price_detail_pro(br_name branch.name%type, month item_sold.month_sold%type) is
	...
	procedure total_sales_pro(br_name branch.name%type, month item_sold.month_sold%type) is
	...
	procedure other_branches_pro(br_name branch.name%type, month item_sold.month_sold%type) is 
	...
	begin
		...
		for i in names loop
			...
			select  sum( distinct item.price * item_sold.quantity)
				...
				dbms_output.put_line('Total sales made in '||i.name||' in the month of '||month||' is FRW '||Total_sales * 1952);
		end loop;

	end other_branches_pro;

	procedure print_report(br_name branch.name%type, month item_sold.month_sold%type) is 
		check_var boolean;
		...
		begin
			check_var := check_branch_Exists_fun(br_name);

			if(check_var = false) then raise_application_error(-20002,'Branch ID does not exist');
			else

			branch_pkg_sec.branch_details_Pro(br_name,month);
			branch_pkg_sec.break_down_pro(br_name,month);
			branch_pkg_sec.price_detail_pro(br_name,month);
			branch_pkg_sec.total_sales_pro(br_name,month);
			branch_pkg_sec.other_branches_pro(br_name,month);
...
begin
	branch_pkg_sec.print_report('Uganda','March');	
end;